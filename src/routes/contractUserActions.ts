...
import { ContractUserAction } from "kybervision20db";
...
router.post("/update-user-favorites", authenticateToken, async (req: Request, res: Response) => {
  console.log("- in POST /contract-user-actions/update-user-favorites");
  try {
    const { sessionId, actionsArray }: { sessionId: number; actionsArray: ActionFavoriteData[] } = req.body;

    // Step 1: Make array of actionIds and favorite status from actionsArray and sessionId
    const actionIdsAndFavoriteStatusArray: ActionFavoriteStatus[] = actionsArray.map((action) => {
      return {
