...
import { ContractPlayerUser } from "kybervision20db";
import { authenticateToken } from "../modules/userAuthentication";

const router = express.Router();

// POST /contract-player-users/link-user-to-player
router.post("/link-user-to-player", authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    let contractPlayerUserObjectUserAlreadyLinked = await ContractPlayerUser.findOne({
...
router.delete("/:playerId", authenticateToken, async (req: Request, res: Response) => {
